...
def vendor_required(f):
    """Require vendor role - FIXED VERSION"""
    @wraps(f)
    def decorated_function(*args, **kwargs):
        # Check if user is logged in and role is vendor
        if 'user_id' not in session:
...
def vendor_dashboard():
    """Vendor dashboard - Corrected for HKO-001 and Phase 1 Requirements"""
    ...
    try:
        ...
        # Profile check: If no vendor record, redirect to create profile
        if not vendor:
            return redirect(url_for('vendor_create_profile'))

        # Get vendor stats: Total Orders and Sales
        stats = cursor.execute("""
            SELECT COUNT(id) as total_orders,
                   IFNULL(SUM(subtotal), 0.0) as total_sales
            FROM order_items
            WHERE vendor_id = ?
        """, (user_id,)).fetchone()

        # Get total income
        income_row = cursor.execute("SELECT IFNULL(SUM(amount), 0.0) FROM commissions WHERE vendor_id = ?", (user_id,)).fetchone()
        ...
        # Preferred Vendor ID for display
        display_id = vendor['vendor_code'] if vendor['vendor_code'] else "HKO-001"

        return render_template('vendor/dashboard.html',
                             vendor=vendor,
                             vendor_id=display_id,
                             products=products,
        ...
        return f"Dashboard currently unavailable: {str(e)}"
    finally:
        db.close()
...
def debug_products():
    ...
    vendor_id = session.get('user_id')
    products = db.execute(
        "SELECT id, name_en, status FROM products WHERE vendor_id = ?", 
        (vendor_id,)
    ).fetchall()
...
def vendor_add_product():
    ...
    db.row_factory = sqlite3.Row
    try:
        # ১. ইনপুট ডাটা নেওয়া (আপনার নতুন HTML ডিজাইন অনুযায়ী)
        name_en = request.form.get('name_en', '').strip()
        name_ar = request.form.get('name_ar', '').strip()
        desc_en = request.form.get('description_en', '').strip()
        desc_ar = request.form.get('description_ar', '').strip()
        
        # প্রাইসিং (নতুন ডিজাইনের ID অনুযায়ী)
        price = float(request.form.get('retail_price', 0)) # HTML-এ retail_price আছে
        cost_price = float(request.form.get('cost_price', 0))
        b2b_price = float(request.form.get('b2b_price', price))
        stock = int(request.form.get('stock_quantity', 0))
        sku = request.form.get('sku', f"HKO-{int(time.time())}") # অটো SKU যদি না থাকে
        
        vendor_id = session.get('user_id')

        # ২. ফাইল হ্যান্ডলিং (ছবি ও ভিডিও)
        main_image = request.files.get('main_image')
        video_file = request.files.get('video_file')
        
        main_image_path = ""
        if main_image and main_image.filename != '':
            main_image_path = save_file(main_image, 'products') # আপনার ফাইল সেভ ফাংশন অনুযায়ী

        video_path = ""
        video_type = 'none'
        if video_file and video_file.filename != '':
            video_path = save_file(video_file, 'videos') # ভিডিও সেভ করা
            video_type = 'file'

        # ৩. ভ্যালিডেশন চেক (HKO-001 এর জন্য জরুরি)
        if not name_en or not name_ar or price <= 0:
            flash('Please fill required fields (Name and Retail Price)!', 'danger')
            return render_template('vendor/add_product.html')
        ...
        sql = """INSERT INTO products
                 (name_en, name_ar, description_en, description_ar, sku, 
                  ...
                  image_url, video_url, video_type, vendor_id, status, created_at) 
                 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', CURRENT_TIMESTAMP)"""
        
        cursor.execute(sql, (
            name_en, name_ar, desc_en, desc_ar, sku,
            price, b2b_price, cost_price, stock,
            main_image_path, video_path, video_type, vendor_id
        ))

        db.commit()
        flash('Success! Product and Video added and waiting for admin review.', 'success')
        return redirect(url_for('vendor_dashboard'))

    except Exception as e:
        db.rollback()
        print(f"Error in vendor_add_product: {str(e)}")
        flash(f'System Error: {str(e)}', 'danger')
        return render_template('vendor/add_product.html')
    finally:
        db.close()
...
@app.route('/vendor/create-profile', methods=['GET', 'POST'])
@vendor_required
def vendor_create_profile():
    ...
    if request.method == 'POST':
        ...
        shop_name_ar = request.form.get('shop_name_ar')
        phone = request.form.get('phone')
        ...
        try:
            db.execute("""
                ...
                SET vendor_name = ?,
                    ...
                    status = 'approved'
                WHERE id = ?
            """, (shop_name_en, shop_name_ar, phone, location, vendor_code, user_id))
            
            db.commit()

            session['vendor_code'] = vendor_code
            session['vendor_name'] = shop_name_en
    ...
    return render_template('vendor/vendor_profile_setup.html')
...
def vendor_orders():
    ...
    query = """
        SELECT oi.*, p.name_en, o.order_number, o.status as order_status
        FROM order_items oi
        JOIN products p ON oi.product_id = p.id
        JOIN orders o ON oi.order_id = o.id
        WHERE p.vendor_id = ?
    ...
    earnings_query = """
        SELECT
            (SELECT IFNULL(SUM(amount), 0.0) FROM commissions 
             WHERE vendor_id = ? AND status = 'completed') as total_earned,
            (SELECT IFNULL(SUM(amount), 0.0) FROM commissions 
             WHERE vendor_id = ? AND status = 'pending') as pending_balance
    """
...
def vendor_withdraw():
    ...
    if request.method == 'POST':
        ...
        db = get_db_connection()
        balance = db.execute(
            'SELECT available_balance FROM vendor_balances WHERE vendor_id = ?',
            (vendor_id,)
        ).fetchone()
    ...
    db = get_db_connection()
    balance = db.execute('SELECT * FROM vendor_balances WHERE vendor_id = ?', (vendor_id,)).fetchone()
    db.close()
